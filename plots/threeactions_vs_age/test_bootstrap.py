...
def bootstrap_error(actionlist, nsample):
    ...
    for i in range(nsample):
        ...
        distribution.append(np.nanmedian(newlist))
    up = np.percentile(distribution,95)
    low = np.percentile(distribution,5)
    return low, up
...
for bin in tqdm(ival):
    ...
    Jrline_lowspread.append(np.percentile(thisJr,10))
    Lzline_lowspread.append(np.percentile(thisLz,10))
    Jzline_lowspread.append(np.percentile(thisJz,10))

    Jrline_upspread.append(np.percentile(thisJr,90))
    Lzline_upspread.append(np.percentile(thisLz,90))
    Jzline_upspread.append(np.percentile(thisJz,90))
