...
for i in range(len(axarr)):
    ...
    with warnings.catch_warnings(record=True):
        ...
        #ax2 = axarr[i].twinx()
        #ax2.bar(1.275,2,0.05,thisage-2)
        #ax2.set(ylim=[0,12])
        #ax2.yaxis.set_ticks([0,2,4,6,8,10,12])
        #if(i==len(axarr)-1):
...
for theta in range(360):
    for i in range(3):
        axarr[i].view_init(azim=theta)        
    fig.subplots_adjust(bottom=0.1)
    #plt.tight_layout()
    plt.savefig('movie/JrLzJz_agecut_'+"{0:0=3d}".format(theta)+'.png', dpi=1000)
